            ...
            {tab === 'project' && (
              <>
                ...
                <Section title="단계(Phase)">
                  <p>기본 14단계: <strong>부품선정 → 회로설계 → 기구설계 → 전장설계 → 구매입고 → 채널조립 → 렉조립 → 채널테스트 → 렉테스트 → 검교정 → 인증 → 출하 → 워런티 → 완료</strong></p>
                  <p>· 검수 사인 시 자동으로 <strong>워런티</strong> 단계로 진입합니다.</p>
                  <p>· 마지막 단계(예: 완료)로 변경하면 진짜 완료 처리됩니다.</p>
                  <p>· 단계 칩을 클릭해 임의 단계로 이동할 수 있습니다.</p>
                  <p>· 단계를 바꾸면 <strong>확인 메시지</strong>가 뜹니다("'A'에서 'B' 단계로 변경합니까?"). 실수 방지 + 변경 이력 자동 기록.</p>
                </Section>
                ...
                <Section title="간트차트 보기">
                  ...
                  <p>· <strong>단계별 탭</strong>: 각 프로젝트가 한 행, 막대는 단계 수 만큼 색 분할(완료 = 진하게 / 현재 = 70% opacity / 예정 = 흐림). 막대 외곽선이 프로젝트 색으로 식별</p>
            ...
            {tab === 'issue' && (
              <>
                ...
                <Section title="완료 처리 모달 — 보고서 첨부 필수">
                  ...
                  <p>· <strong>완료 시 처리 유형 / 비용 청구 선택</strong> — 완료 모달에서 처리 유형(HW: 단순점검·부품교체·장비교체·기타 / SW: 원격지원·패치적용·설정변경·기타)과 비용 청구(무상(보증기간 내) / 유상(청구 대상))를 함께 입력합니다.</p>
                  <p>· 완료 후 카드에 <strong>처리유형·비용청구 배지</strong>(유상=빨강/무상=초록) 표시, 상단에 완료 건 무상/유상 카운트, AS 보고서 메일·Excel 내보내기에도 자동 포함됩니다.</p>
                </Section>
                <Section title="AS 연도 관리 — 연간 마감 / 연도별 조회 / 통합검색">
                  <p>AS 통합 관리 헤더에서 연도 단위로 데이터를 정리·조회할 수 있습니다.</p>
                  <p>· <strong>연간 마감</strong> (ADMIN) — 특정 연도의 완료 AS를 보관 처리. 기본 목록에서 숨겨져 현재 진행분만 깔끔하게 보입니다. 되돌리기(마감 취소) 가능.</p>
                  <p>· <strong>연도 셀렉터</strong> — "현재 진행중(미마감)" 또는 마감된 연도를 선택해 그 해 보관분을 열람(읽기 전용)할 수 있습니다.</p>
                  <p>· <strong>통합검색</strong> — 연도·마감 구분 없이 전체 AS에서 검색. 과거 이력을 빠르게 찾을 때 사용합니다.</p>
                </Section>
                <Section title="기존 AS 데이터 가져오기 (이관)">
                  <p>기존에 별도로 관리하던 AS 통합관리(엑셀/CSV)를 현 시스템으로 옮길 수 있습니다 (ADMIN/PM).</p>
                  <p>· 헤더 <code className="bg-blue-600 text-white px-1 rounded text-[10px]">AS 데이터 가져오기</code> → 파일 선택 → <strong>미리보기</strong>(이관 대상 / 기존 매칭 / 신규 프로젝트 수 / 중복·삭제행)에서 확인 후 적용합니다.</p>
                  <p>· 기존 <strong>장비 코드</strong>와 일치하는 프로젝트가 있으면 그 프로젝트로, 없으면 <strong>코드별로 프로젝트를 새로 생성</strong>(이름=고객사, 코드=장비 코드 등록)해 해당 코드의 AS를 넣습니다.</p>
                  <p>· 동일 원본은 다시 가져와도 중복되지 않습니다(멱등). 이관된 AS 카드에는 파란 <strong>원본코드 배지</strong>가 표시됩니다.</p>
                </Section>
                <Section title="이관 AS 프로젝트 연결">
                  <p>이관된 AS 카드의 <code className="bg-blue-50 text-blue-700 px-1 rounded text-[10px]">프로젝트 연결</code> 버튼(ADMIN/PM)으로 추적성을 보정할 수 있습니다.</p>
                  <p>· <strong>① 기존 프로젝트에 연결</strong> — 같은 코드 그룹을 선택한 프로젝트로 이동(장비 코드 일치 시 ★ 자동 추천)</p>
                  <p>· <strong>② 이 코드로 새 프로젝트 생성</strong> — 코드를 장비 코드로 등록한 새 프로젝트 생성 후 해당 AS 이동</p>
                  <p>· <strong>③ 현재 프로젝트 정보 수정</strong> — 새로 만들지 않고 이름·고객사·장비 코드를 바로 수정</p>
                </Section>
            ...
            {tab === 'changelog' && (
              <>
                ...
                <Section title={t('간트차트 — 시각 강화', 'Gantt — Visual Enhancements')}>
                  ...
                  <p>· <strong>단계별 탭 막대 분할</strong> — 단계 수 만큼 색 분할(완료=진하게 / 현재=70% / 예정=흐림), 외곽선이 프로젝트 색</p>
