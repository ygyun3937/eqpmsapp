...
function DashboardView({ projects, issues, engineers, getStatusColor, calcExp, calcAct, t }) {
  ...
  const handleExportSummary = () => {
    const summaryData = [
      ...
      { category: '완료된 프로젝트 수', count: projectStats.find(s=>s.label==='완료')?.value || 0 },
...
function ProjectListView({ projects, issues, getStatusColor, onAddClick, onManageTasks, onEditVersion, onDeleteProject, onUpdatePhase, onIssueClick, calcExp, calcAct, currentUser, t }) {
  ...
  const handleExportProjects = () => {
    ...
    exportToCSV(exportData, '프로젝트_리스트', [
      { header: '프로젝트 ID', key: 'id' },
      { header: '산업군', key: 'domain' },
      { header: '프로젝트명', key: 'name' },
      { header: '고객사', key: 'customer' },
      { header: '사이트', key: 'site' },
      { header: '담당자', key: 'manager' },
      { header: '시작일', key: 'startDate' },
      { header: '납기일', key: 'dueDate' },
      { header: '상태', key: 'status' },
      { header: '현재 진척도', key: 'progress' }
    ]);
  };
  // =============================================
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        ...
        <div className="flex items-center space-x-3">
          ...
          {/* ==== 엑셀 다운로드 버튼 추가 ==== */}
          <button onClick={handleExportProjects} className="flex items-center bg-slate-100 text-slate-600 border border-slate-200 px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-slate-200 transition-colors shadow-sm">
...
export default function App() {
  ...
  const GAS_URL = "https://script.google.com/macros/s/AKfycbwVargrc_T8Gw-GJeqv1WtsmiRv5i62lhqJ60ut8aOjhAkBtDR8Ztl7cVfoHtmn4mfh3g/exec"; 
  ...
  const saveToGoogleDB = async (type, payload) => {
    if (!GAS_URL) return;
    try {
      await fetch(GAS_URL, {
        method: 'POST',
  ...
  const handleAddIssue = async (newIssue) => {
    ...
    setIssues([issueWithDetails, ...issues]); setIsIssueModalOpen(false); 
  ...
  const handleAddProject = async (newProject) => {
    ...
    saveToGoogleDB('ADD_PROJECT', newData[0]);
  };

  const handleUpdatePhase = async (projectId, newPhaseIndex) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, phaseIndex: newPhaseIndex, status: newPhaseIndex === 6 ? '완료' : '진행중' } : p)); 
  ...
  const handleDeleteProject = async () => { 
    ...
    setProjectToDelete(null); showToast('프로젝트가 삭제되었습니다.'); 
  };
  
  const toggleTaskCompletion = (projectId, taskId) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, tasks: p.tasks.map(t => t.id === taskId ? { ...t, isCompleted: !t.isCompleted } : t) } : p)); 
    saveToGoogleDB('TOGGLE_TASK', { projectId, taskId });
  };
  const handleAddTask = (projectId, taskName) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, tasks: [...p.tasks, { id: Date.now(), name: taskName, isCompleted: false, delayReason: '' }] } : p)); 
    saveToGoogleDB('ADD_TASK', { projectId, taskName });
  };
  const handleEditTaskName = (projectId, taskId, newName) => { 
    if (!newName.trim()) return; 
    setProjects(projects.map(p => p.id === projectId ? { ...p, tasks: p.tasks.map(t => t.id === taskId ? { ...t, name: newName.trim() } : t) } : p)); 
  ...
  const handleUpdateDelayReason = (projectId, taskId, reason) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, tasks: p.tasks.map(t => t.id === taskId ? { ...t, delayReason: reason } : t) } : p)); 
  ...
  const handleUpdateChecklistItem = (projectId, itemId, newStatus, newNote) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, checklist: p.checklist.map(c => c.id === itemId ? { ...c, status: newStatus, note: newNote !== undefined ? newNote : c.note } : c) } : p)); 
    saveToGoogleDB('UPDATE_CHECKLIST', { projectId, itemId, newStatus, newNote });
  };
  const handleLoadDefaultChecklist = (projectId) => { 
    setProjects(projects.map(p => { if (p.id !== projectId) return p; const domainChecklist = DOMAIN_CHECKLIST[p.domain] || DOMAIN_CHECKLIST['반도체']; return { ...p, checklist: domainChecklist.map((item, idx) => ({ ...item, id: Date.now() + idx })) }; })); showToast('기본 검수표가 불러와졌습니다.'); 
    saveToGoogleDB('LOAD_DEFAULT_CHECKLIST', { projectId });
  };
  const handleAddChecklistItem = (projectId, category, taskName) => { 
    if (!taskName.trim()) return; 
    setProjects(projects.map(p => p.id === projectId ? { ...p, checklist: [...(p.checklist || []), { id: Date.now(), category: category || '일반', task: taskName.trim(), status: 'Pending', note: '' }] } : p)); 
  ...
  const handleDeleteChecklistItem = (projectId, itemId) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, checklist: (p.checklist || []).filter(c => c.id !== itemId) } : p)); 
    saveToGoogleDB('DELETE_CHECKLIST_ITEM', { projectId, itemId });
  };
  const handleSignOff = async (projectId, customerName, signatureData) => { 
    const todayStr = new Date().toISOString().split('T')[0];
    setProjects(projects.map(p => p.id === projectId ? { ...p, status: '완료', phaseIndex: 6, signOff: { signed: true, customerName, signatureData, date: todayStr } } : p)); showToast('✅ 최종 검수 및 서명이 완료되었습니다!'); 
    saveToGoogleDB('SIGN_OFF', { projectId, customerName, signatureData, date: todayStr });
  };

  const handleAddPart = async (newPart) => { 
    const selectedProject = projects.find(p => p.id === newPart.projectId); const partWithDetails = { ...newPart, id: generateUniqueId('PRT'), projectName: selectedProject ? selectedProject.name : t('알 수 없는 프로젝트', 'Unknown Project'), date: TODAY.toISOString().split('T')[0], status: '청구' }; setParts([partWithDetails, ...parts]); setIsPartModalOpen(false); showToast(t('자재 청구가 접수되었습니다.')); 
    saveToGoogleDB('ADD_PART', partWithDetails);
  };
  const handleDeletePart = async () => { 
    if (!partToDelete) return; setParts(parts.filter(p => p.id !== partToDelete.id)); 
    saveToGoogleDB('DELETE_PART', { partId: partToDelete.id });
    setPartToDelete(null); showToast(t('자재 청구 내역이 삭제되었습니다.')); 
  };
  ...
  const handleAddSite = async (newSite) => { 
    ...
    setSites(selectedSite ? sites.map(s => s.id === selectedSite.id ? newSiteData : s) : [newSiteData, ...sites]); setIsSiteModalOpen(false); showToast(t('사이트가 업데이트되었습니다.')); 
    saveToGoogleDB(selectedSite ? 'UPDATE_SITE' : 'ADD_SITE', newSiteData);
  };
  const handleDeleteSite = async () => { 
    if (!siteToDelete) return; setSites(sites.filter(s => s.id !== siteToDelete.id)); 
    saveToGoogleDB('DELETE_SITE', { siteId: siteToDelete.id });
    setSiteToDelete(null); showToast(t('사이트 환경 정보가 삭제되었습니다.')); 
  };
  
  const handleAddRelease = async (newRelease) => { 
    const releaseData = { ...newRelease, id: generateUniqueId('REL'), date: TODAY.toISOString().split('T')[0] };
    setReleases([releaseData, ...releases]); setIsReleaseModalOpen(false); showToast(t('릴리즈 정보가 등록되었습니다.')); 
    saveToGoogleDB('ADD_RELEASE', releaseData);
  };
  const handleDeleteRelease = async () => { 
    if (!releaseToDelete) return; setReleases(releases.filter(r => r.id !== releaseToDelete.id)); 
    saveToGoogleDB('DELETE_RELEASE', { releaseId: releaseToDelete.id });
    setReleaseToDelete(null); showToast(t('버전 이력이 삭제되었습니다.')); 
  };
  ...
  const handleAddEngineer = async (engineerData) => { 
    ...
    setEngineers(selectedEngineer ? engineers.map(e => e.id === selectedEngineer.id ? engData : e) : [engData, ...engineers]); setIsEngineerModalOpen(false); showToast(t('엔지니어 정보가 업데이트되었습니다.')); 
    saveToGoogleDB(selectedEngineer ? 'UPDATE_ENGINEER' : 'ADD_ENGINEER', engData);
  };
  const handleDeleteEngineer = async () => { 
    if (!engineerToDelete) return; setEngineers(engineers.filter(e => e.id !== engineerToDelete.id)); 
    saveToGoogleDB('DELETE_ENGINEER', { engineerId: engineerToDelete.id });
    setEngineerToDelete(null); showToast(t('엔지니어 정보가 삭제되었습니다.')); 
  };
  
  const handleUpdateVersion = async (projectId, hwVersion, swVersion, fwVersion) => { 
    setProjects(projects.map(p => p.id === projectId ? { ...p, hwVersion, swVersion, fwVersion } : p)); setIsVersionModalOpen(false); showToast(t('버전이 업데이트되었습니다.')); 
    saveToGoogleDB('UPDATE_VERSION', { projectId, hwVersion, swVersion, fwVersion });
  };
  const handleAddDailyReport = async (reportData) => { 
    setIsDailyReportOpen(false); showToast(t('일일 보고서가 제출되었습니다.')); 
  ...
  const handleAddComment = async (issueId, text) => { 
    ...
    setIssues(issues.map(issue => issue.id === issueId ? { ...issue, comments: [...(issue.comments || []), commentData] } : issue)); 
  ...
  const handleDeleteIssue = async () => { 
    if (!issueToDelete) return; setIssues(issues.filter(i => i.id !== issueToDelete.id)); 
    saveToGoogleDB('DELETE_ISSUE', { issueId: issueToDelete.id });
    setIssueToDelete(null); showToast(t('이슈가 삭제되었습니다.')); 
  };
